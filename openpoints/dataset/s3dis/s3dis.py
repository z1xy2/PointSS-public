...
class S3DIS(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        if self.presample:
            ...
            cdata = np.load(data_path).astype(np.float32)#取得原始数据
            cdata[:, :3] -= np.min(cdata[:, :3], 0)#将坐标原点移动到最小值
            coord, feat, label = cdata[:, :3], cdata[:, 3:6], cdata[:, 6:7]#分别取出，coord坐标，feat颜色，label标签1
        ...
        data = {'pos': coord, 'x': feat, 'y': label}#pos为xyz坐标，x为颜色，y为标签
        # pre-process.
        if self.transform is not None:
            data = self.transform(data)

        if 'heights' not in data.keys():
            data['heights'] =  torch.from_numpy(coord[:, self.gravity_dim:self.gravity_dim+1].astype(np.float32))#把变换前点的z坐标单独取出，作为height
        return data
