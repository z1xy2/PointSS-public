...
class PointMambaEncoder(nn.Module):
    def __init__(self, in_channels=3, embed_dim=64, groups=1, res_expansion=1.0,
        ...
        if use_order_prompt:
            ...
            for order in mamba_layers_orders:
                self.per_layer_prompt_indexe.append(order2idx[order])#根据order在数组中的顺序，将索引存入数组
    ...
    # 对点云进行序列化，返回排序之后的点云，比如xyz，就是X最优先降序，X相同再比较Y
    ...
    def forward_seg_feat(self, p, x=None):
        self.order = "original"
        if isinstance(p, dict):
            p, x = p['pos'], p.get('x', None)#p转换为坐标
        if x is None:
            x = p.transpose(1, 2).contiguous()
        else:
            if self.combine_pos:
                x = torch.cat([x, p.transpose(1, 2)], dim=1).contiguous()#将坐标和特征拼接在一起
        batch_size, _, _ = x.size()
        x = self.embedding(x)  # B,D,N 对特征进行一次编码，得到新特征

        p_list, x_list = [p], [x]#坐标与特征
        ...
        for i in range(self.stages):
            ...
            p, x, x_res = self.local_grouper_list[i](p, x.permute(0, 2, 1), x_res)  # [b,g,3]  [b,g,k,d] 首先进行下采样
            x = self.pre_blocks_list[i](x)  # [b,d,g] b：batch_size，d：特征维度，g：点的数量，也就是简单特征提取

            x = x.permute(0, 2, 1).contiguous() #[[b,g,d]
            if not self.block_residual:
                x_res = None
            x_res = self.residual_proj_blocks_list[i](x_res)
            for layer in self.mamba_blocks_list[i]:#mamba层
                p, x, x_res = self.serialization_func(p, x, x_res, self.mamba_layers_orders[mamba_layer_idx])#进行序列化排序 todo:查看残差
                if self.use_windows:#是分patch，P：[24,1024,3],24是所有batch整合后的patch数，1024是一个patch中的点，3是坐标
                    p, x, x_res, n_windows, p_base, p_std = self.pre_split_windows(
                        p, x, x_res, windows_size=self.windows_size[i])
                if self.mamba_pos:#位置编码，将坐标通过线性变换后加到特征上
                    if self.pos_type == 'share':
                        if self.block_pos_share:
                            x = x + self.pos_proj(p)
                        else:
                            x = x + self.pos_proj[i](p)#走这个
                    else:
                        x = x + self.pos_proj[pos_proj_idx](p)
                        pos_proj_idx += 1
                if self.use_order_prompt:#是否使用顺序提示
                    layer_order_prompt_indexes = self.per_layer_prompt_indexe[mamba_layer_idx]#取出第i个顺序提示的index
                    ...
                    #layer_order_prompt存储顺序标识，是可学习的参数，一个order是6个，每个维度默认为384.
                    layer_order_prompt = self.order_prompt_proj[i](layer_order_prompt)#对layer_order_prompt权重参数进行变换，使其维度变变化
                    layer_order_prompt = layer_order_prompt.unsqueeze(0).repeat(p.shape[0], 1, 1)#【24，6，96】patch数，6个权重，维度
                    x = torch.cat([layer_order_prompt, x, layer_order_prompt], dim=1)#很神奇，将顺序提示当做点给添加进去了，前面六个点，后面六个点，实际都是顺序提示
                    if x_res is not None:
                        x_res = torch.cat([layer_order_prompt, x_res, layer_order_prompt], dim=1)
                    x, x_res = layer(x, x_res)#进入进行训练
                    x = x[:, self.promot_num_per_order:-self.promot_num_per_order]#出来时将前后的顺序提示去掉
                    x_res = x_res[:, self.promot_num_per_order:-self.promot_num_per_order]#残差同样
                else:
                    x, x_res = layer(x, x_res)
                if self.use_windows: #将被分割为窗口的数据合并
                    p, x, x_res = self.post_split_windows(p, x, x_res, n_windows, p_base, p_std)
                mamba_layer_idx += 1
            x = x.permute(0, 2, 1).contiguous()
            x = self.pos_blocks_list[i](x)  # [b,d,g] 没用，全是Identity

            p_list.append(p)#加入list
            x_list.append(x)#加入list

        return p_list, x_list

    def pre_split_windows(self, p, x, x_res, windows_size=1024):
        #将输入的数据 p、x 和 x_res 按照窗口大小 windows_size 进行切分。如果输入数据的大小超过指定的窗口大小，
        # 它会通过一种叫“最远点采样”（FPS，Furthest Point Sampling）的方式进行采样，之后再将数据切分成多个窗口进行处理。
        # x (bs, n, c), p (bs, n, 3)
        bs, n, c = x.shape
        if n <= windows_size:
            return p, x, x_res, 1, 0, 1

        # fps sample：用于将点变为windows_size的倍数，n_sample<n。
        n_sample = n // windows_size * windows_size #n_sample 表示要采样的点数，是原始点数 n 中最多能被 windows_size 整除的部分。
        fps_idx = furthest_point_sample(p, n_sample).long()  # [B, n_windows] 最远点采样取点
        fps_idx = torch.sort(fps_idx, dim=-1)[0] #对batch的idx进行排序
