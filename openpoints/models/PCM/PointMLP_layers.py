...
class LocalGrouper(nn.Module):
    ...
    def forward(self, xyz, points, points_res):#进行下采样，类似于PointNet++的下采样，没啥区别
        B, N, C = xyz.shape
        S = N // self.sample_ratio#sample_ratio为，下采样比率，S为下一层的点数量
        ...
        if S == N:
            ...
            new_points = index_points(points, fps_idx)  # [B, npoint, d] 取出下采样的点，按index排序，new_points为新的下采样点
            if points_res is not None:
                points_res = index_points(points_res, fps_idx)

        idx = knn_point(self.kneighbors, xyz, new_xyz, training=self.training)#取出每个下采样点的k个最近邻点
        idx = idx[:, :, ::self.k_stride]#取出每个下采样点的k个最近邻点，步长为k_stride（一般stride为1）
        grouped_xyz = index_points(xyz, idx)  # [B, npoint, k, 3]
        grouped_points = index_points(points, idx)  # [B, npoint, k, d]#取点，每个新点的周围的点
        if self.use_xyz:
        ...
        if self.normalize is not None:
            ...
            if self.normalize == "anchor":
                mean = torch.cat([new_points, new_xyz], dim=-1) if self.use_xyz else new_points #如果使用xyz则进行拼接
                mean = mean.unsqueeze(dim=-2)  # [B, npoint, 1, d+3] 同一组取均值
            std = torch.std((grouped_points - mean).reshape(B, -1), dim=-1, keepdim=True).unsqueeze(dim=-1).unsqueeze(
                dim=-1)#grouped_points - mean将值中心化，指定了沿最后一个维度计算标准差，即对展平后的每个批次计算每个特征的标准差
            grouped_points = (grouped_points - mean) / (std + 1e-5)#进行一个缩放
            grouped_points = self.affine_alpha * grouped_points + self.affine_beta#没用
