...
class NormalizeColor(object):
    def __call__(self, data_dict):
        if "color" in data_dict.keys():
            ...
            # myCombined(data_dict)   #训练时需要关闭
        return data_dict

def myCombined(data_dict):
    combined = np.concatenate((data_dict["coord"], data_dict["color"]), axis=1)
    np.save(os.path.join("./processed_data_vis",data_dict["name"]), combined)
