...
class SerializedAttention(PointModule):
    ...
    @torch.no_grad()
    def get_padding_and_inverse( self,point, current_patch_size,config_patch_size):
        pad_key = "pad"
        unpad_key = "unpad"
        ...
        if (
            ...
            mask_pad = bincount > current_patch_size  # 该位置点数是否大于self.patch_size True or False
            # 通过mask_pad调整bincount_pad，确保仅对元素数量超过patch_size的块进行填充，元素不超过。
            bincount_pad = ~mask_pad * current_patch_size + mask_pad * bincount_pad
            ...
            for i in range(len(offset)):  # 每一个batch遍历。是一个一维数组，表示每个点云批次的起始索引。
                ...
                unpad[_offset[i]: _offset[i + 1]] += _offset_pad[i] - _offset[i]#计算unpad_key
                ...
                if bincount[i] != bincount_pad[i]:#如果需要填充,填充与原版ptv3略有区别
                    ...
                    pad[
                        ...
                        : _offset_pad[i + 1]
                            ...
                            - 2 * current_patch_size
                            + (bincount[i] % current_patch_size): _offset_pad[i + 1]
                                                                    - current_patch_size+ (bincount[i] % current_patch_size)
                    ]


                # 更新填充（pad）数组，调整填充后序列中的索引，以确保它们正确反映原始数据中的位置。
    ...
    def forward(self, point):
        ...
        H = self.num_heads
        # K为当前patch,会动态变化
        if (bincount.min() < self.current_patch_size):  # 如果每个batch的点数都大于patch_size，那么patch_size就是我们预先设置好的最大值，也就是self.patch_size
            K = bincount.min()  # 如果有任何一个batch的点连一个patch_size都不到，那么patch_size就是这个batch的最少点数。
        else:
            K = self.current_patch_size
        C = self.channels #通道数
        ...
        pad, unpad, cu_seqlens = self.get_padding_and_inverse(point,K,self.current_patch_size)
        ...
        x,x_res=self.mamba(x,x_res) #todo：看x_res如何修改,这个不好改，还要在下采样里改，先不改试试。
        x = x[:, self.prompt_num_per_order:-self.prompt_num_per_order]  # 出来时将前后的顺序提示去掉
        x=x.reshape(-1, C) #返回为非patch的情况
        feat = x[inverse]#将排好序和pad的点云转换为最初点云的顺序（没排序没pad）
