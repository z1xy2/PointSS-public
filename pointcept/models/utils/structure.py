...
class Point(Dict):
    ...
    def serialization(self, order="z", depth=None, shuffle_orders=False):
        ...
        if depth is None:#如果未指定 depth，则自适应地计算序列化立方体的深度（depth），该深度为序列化码的最大位长。
        ...
        # 将四种排序的序列化码整体进行移动，比如现在第一行是z序列化码，第二行是z-trans序列化码，第三行是hilbert序列
        if shuffle_orders:#是否打乱排序
            perm = torch.randperm(code.shape[0])
            code = code[perm]
            order = order[perm]
            inverse = inverse[perm]
            sort_name= [sort_name[i.item()] for i in perm]
